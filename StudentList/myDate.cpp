...
int myDate::Greg2Julian(int month, int day, int year) // calculations @.@
{
...
void myDate::Julian2Greg(int JD, int& month, int& day, int& year)// calculations @.@
{
    int L, N, I, J, K;
    L = JD + 68569;
    N = 4 * L / 146097;
...
{
    myDate date1(year, 3, 1);
    myDate date2(year, 2, 28);
    if (date2.Greg2Julian(date2.getMonth(), date2.getDay(), date2.getYear()) - date1.Greg2Julian(date1.getMonth(), date1.getDay(), date1.getYear()))
    {
        return true;
    }
    return false;
...
void myDate::display() // create a display 
{
...
int myDate::daysBetween(myDate D) //subtract the julian days 
{
...
int myDate::dayOfYear()  // we only need to find out which day it is 
{
...
string myDate::toString() {
    ...
    toString << months[month - 1] << " " << day << ", " << year;
    return toString.str();
}
